...
contract BME is ERC20, Ownable {
    ...
    uint256 public STatAmt    = 500000000 * (10**18);
    ...
    function _transfer(
        ...
        if(
            ...
            if ( totalSupply().sub(tokenstoBurn) >= MtotalS ){
                AutoBurn(tokenstoBurn);
    ...
    function AutoBurn(uint256 amount) private {

        _burn( address(this) , amount );

        emit BurnedTokens( address(this) , amount );
    }

    function Burn(uint256 amount) public onlyOwner{
    require( totalSupply().sub(amount) >= MtotalS , "Total Supply should be more than 10B token.");

        _burn( Lwallet , amount );

        emit BurnedTokens( Lwallet , amount );
    }
    ...
    function SWandSendBTCd(uint256 BTCtokens , uint256 BMEtokens) private {
...
contract BBTracking is DividendPayingToken{
    using SafeMath for uint256;
